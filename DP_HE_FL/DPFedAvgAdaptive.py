...
class DPFedAvgAdaptive(DPFedAvgFixed):
    ...
    def __init__(
        ...
        # Audris: Added a fix for complex number creation
        #  in the default code
        part_one = self.noise_multiplier ** (-2)
        log(INFO, "one type = " + str(type(part_one)))
        log(INFO, "one type = " + str(part_one))

        part_two = (2 * self.clip_count_stddev) ** (-2)
        log(INFO, "two type = " + str(type(part_two)))
        log(INFO, "two type = " + str(part_two))

        part_three = part_one - part_two
        log(INFO, "three type = " + str(type(part_three)))
        log(INFO, "three type = " + str(part_three))

        part_four = None
        if part_three < 0:
            part_three = -part_three
            part_four = -(part_three ** -0.5)
        else:
            part_four = (part_three ** -0.5)

        log(INFO, "four type = " + str(type(part_four)))
        log(INFO, "four type = " + str(part_four))

        self.noise_multiplier = part_four

        # Audris: Python returned complex numbers for negative number power with -0.5
        ...
        log(INFO, "after nm type = " + str(type(self.noise_multiplier)))

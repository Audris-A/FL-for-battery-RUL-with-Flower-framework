...
class BatteryClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.model.fit(self.train_x, self.train_y, epochs=self.epochs, batch_size=32, validation_data=(self.val_x, self.val_y))
    ...
    def evaluate(self, parameters, config):
        ...
        log(INFO, str((loss, len(self.test_x), {"mse": float(mse), "mae": float(mae), "mape": float(mape), "rmse": float(rmse)})))

        return loss, len(self.test_x), {"mse": float(mse), "mae": float(mae), "mape": float(mape), "rmse": float(rmse)}